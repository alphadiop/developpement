...
def index(request):
    ...
    return render(request,"booking/index.html",{"clients":clients})


def api_get_clients(request):
    clients = Booking.objects.all()
    json = serializers.serialize("json",clients)
...
def view_form(request):
  if request.method == "POST":
    ...
    if form.is_valid():
      ...
      form = BookingForm()
  return render(request, 'booking/forms.html', {'form': form})
...
class BookingHomeView(BookingSettingMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["last_bookings"] = Booking.objects.filter().order_by("date", "time")[:10]
...
def get_available_time(date: datetime.date) -> List[Dict[datetime.time, bool]]:
    ...
    """
    booking_settings = BookingSettings.objects.first()
    existing_bookings = Booking.objects.filter(date=date).values_list('time')

    next_time = booking_settings.start_time
    time_list = []
    while True:
        is_taken = any([x[0] == next_time for x in existing_bookings])
        time_list.append({"time": ":".join(str(next_time).split(":")[:-1]), "is_taken": is_taken})
        next_time = add_delta(next_time, datetime.timedelta(minutes=int(booking_settings.period_of_each_booking)))
